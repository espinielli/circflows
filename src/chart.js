...
(function( scope ) {
  ...
  scope.chart = function( data, config ) {
    ...
    config.outerRadius = config.outerRadius ||
      (Math.min( config.width, config.height ) / 2 - config.margin);
    ...
    config.initialAngle.chord = config.initialAngle.chord ||
      { source: config.initialAngle.arc, target: config.initialAngle.arc };
    ...
    // start angle for first region (0, zero, is up North)
    config.layout.alpha = config.layout.alpha || aLittleBit;
    ...
    function chordInfo( d ) {
      ...
      infoTimer = setTimeout(function() {
        ...
        var text = info.select( ".text" ).selectAll( "text" )
          .data([
            data.names[ d.source.id ] + " → " + data.names[ d.target.id ] + ": " +
    ...
    function draw( year, countries ) {
      ...
      groupPath
        ...
        .attrTween("d", function( d ) {
          var i = d3.interpolate(
            previous.groups[ d.id ] ||
            previous.groups[ d.region ] ||
            meltPreviousGroupArc( d ) ||
            config.initialAngle.arc, d );
      ...
      groupTextGroup
        ...
        .attrTween("transform", function( d ) {
          var i = d3.interpolate(
            ...
            { angle: 0 }, d );
          return function( t ) {
            var p = labelPosition( i( t ).angle );
            return "translate(" + p.x + " " + p.y + ") rotate(" + p.r + ")";
          };
        });
      groupTextGroup.exit()
        .transition()
      ...
      groupTextPath
        .filter(function( d, i ) {
          ...
          var labelArcLength = (d.endAngle - d.startAngle) * (config.outerRadius + 18);
          return this.getComputedTextLength() > labelArcLength;
        })
        .remove();
      ...
      chord
        ...
        .attrTween("d", function( d ) {
          ...
          p = p || (previous.chords[ d.source.region ] &&
                    previous.chords[ d.source.region ][ d.target.region ]);
